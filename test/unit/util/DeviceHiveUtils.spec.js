...
describe(DeviceHiveUtils.name, () => {
    ...
    describe(`Static method: ${staticMethodsNames.createSubscriptionDataObject}`, () => {
        ...
        it(`subscription data object for "${expectationObject1.topic}"`, () => {
            expect(
                DeviceHiveUtils.createSubscriptionDataObject(
                    expectationObject1.topic
                )
    ...
    describe(`Static method: ${staticMethodsNames.isSameTopicRoot}`, () => {
        ...
        it(`"${topic1}" and "${topic2}" has different topic root`, () => {
            expect(DeviceHiveUtils.isSameTopicRoot(topic1, topic2)).to.equal(
                false
            );
        });

        it(`"${topic2}" and "${topic3}" has different topic root`, () => {
            expect(DeviceHiveUtils.isSameTopicRoot(topic2, topic3)).to.equal(
                false
            );
        });

        it(`"${topic3}" and "${topic4}" has same topic root`, () => {
            expect(DeviceHiveUtils.isSameTopicRoot(topic3, topic4)).to.equal(
                true
            );
        });

        it(`"${topic4}" and "${topic5}" has same topic root`, () => {
            expect(DeviceHiveUtils.isSameTopicRoot(topic4, topic5)).to.equal(
                true
            );
        });
    });
    ...
    describe(`Static method: ${staticMethodsNames.isLessGlobalTopic}`, () => {
        ...
        it(`"${topic1}" should be less global than "${topic2}"`, () => {
            expect(DeviceHiveUtils.isLessGlobalTopic(topic1, topic2)).to.equal(
                true
            );
        });

        it(`"${topic2}" should be less global than "${topic3}"`, () => {
            expect(DeviceHiveUtils.isLessGlobalTopic(topic2, topic3)).to.equal(
                true
            );
        });

        it(`"${topic3}" should not be less global than "${topic4}"`, () => {
            expect(DeviceHiveUtils.isLessGlobalTopic(topic3, topic4)).to.equal(
                false
            );
        });

        it(`"${topic4}" should be less global than "${topic5}"`, () => {
            expect(DeviceHiveUtils.isLessGlobalTopic(topic4, topic5)).to.equal(
                true
            );
        });

        it(`"${topic3}" should be less global than "${topic6}"`, () => {
            expect(DeviceHiveUtils.isLessGlobalTopic(topic3, topic6)).to.equal(
                true
            );
        });
    });
    ...
    describe(`Static method: ${staticMethodsNames.isMoreGlobalTopic}`, () => {
        ...
        it(`"${topic1}" should not be more global than "${topic2}"`, () => {
            expect(DeviceHiveUtils.isMoreGlobalTopic(topic1, topic2)).to.equal(
                false
            );
        });

        it(`"${topic3}" should be more global than "${topic2}"`, () => {
            expect(DeviceHiveUtils.isMoreGlobalTopic(topic3, topic2)).to.equal(
                true
            );
        });

        it(`"${topic3}" should not be more global than "${topic4}"`, () => {
            expect(DeviceHiveUtils.isMoreGlobalTopic(topic3, topic4)).to.equal(
                false
            );
        });

        it(`"${topic4}" should not be more global than "${topic5}"`, () => {
            expect(DeviceHiveUtils.isMoreGlobalTopic(topic4, topic5)).to.equal(
                false
            );
        });

        it(`"${topic5}" should be more global than "${topic3}"`, () => {
            expect(DeviceHiveUtils.isMoreGlobalTopic(topic5, topic3)).to.equal(
                true
            );
        });
    });
    ...
    describe(`Static method: ${staticMethodsNames.getTopicSubscribeRequestAction}`, () => {
        ...
        it(`Action for "${expectation1[0]}" should be "${expectation1[1]}"`, () => {
            expect(
                DeviceHiveUtils.getTopicSubscribeRequestAction(expectation1[0])
            ).to.equal(expectation1[1]);
        });

        it(`Action for "${expectation2[0]}" should be "${expectation2[1]}"`, () => {
            expect(
                DeviceHiveUtils.getTopicSubscribeRequestAction(expectation2[0])
            ).to.equal(expectation2[1]);
        });

        it(`Action for "${expectation3[0]}" should be "${expectation3[1]}"`, () => {
            expect(
                DeviceHiveUtils.getTopicSubscribeRequestAction(expectation3[0])
            ).to.equal(expectation3[1]);
        });

        it(`Action for "${expectation4[0]}" should be "${expectation4[1]}"`, () => {
            expect(
                DeviceHiveUtils.getTopicSubscribeRequestAction(expectation4[0])
            ).to.equal(expectation4[1]);
        });
    });
    ...
    describe(`Static method: ${staticMethodsNames.getTopicUnsubscribeRequestAction}`, () => {
        ...
        it(`Action for "${expectation1[0]}" should be "${expectation1[1]}"`, () => {
            expect(
                DeviceHiveUtils.getTopicUnsubscribeRequestAction(
                    expectation1[0]
                )
            ).to.equal(expectation1[1]);
        });
    ...
    describe(`Static method: ${staticMethodsNames.getTopicSubscriptionResponseAction}`, () => {
        ...
        it(`Action for "${expectation1[0]}" should be "${expectation1[1]}"`, () => {
            expect(
                DeviceHiveUtils.getTopicSubscriptionResponseAction(
                    expectation1[0]
                )
            ).to.equal(expectation1[1]);
        });
