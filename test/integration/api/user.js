...
const GET_FACILITY_OPERATION = `getFacility`;
const ASSIGN_FACILITY_OPERATION = `assignFacility`;
const UNASSIGN_FACILITY_OPERATION = `unassignFacility`;
...
const GET_FACILITY_ACTION = `${SUBJECT}/${GET_FACILITY_OPERATION}`;
const ASSIGN_FACILITY_ACTION = `${SUBJECT}/${ASSIGN_FACILITY_OPERATION}`;
const UNASSIGN_FACILITY_ACTION = `${SUBJECT}/${UNASSIGN_FACILITY_OPERATION}`;
...
const GET_FACILITY_TOPIC = `${CONST.DH_RESPONSE_TOPIC}/${GET_FACILITY_ACTION}`;
const ASSIGN_FACILITY_TOPIC = `${CONST.DH_RESPONSE_TOPIC}/${ASSIGN_FACILITY_ACTION}`;
const UNASSIGN_FACILITY_TOPIC = `${CONST.DH_RESPONSE_TOPIC}/${UNASSIGN_FACILITY_ACTION}`;
...
it(`should subscribe for "${GET_FACILITY_TOPIC}" topic`, () => {
    return new Promise((resolve, reject) => {
        mqttClient.subscribe(
            `${GET_FACILITY_TOPIC}@${mqttClient.options.clientId}`,
            (err) => {
                if (err) {
                    reject(err);
                } else {
...
it(`should subscribe for "${ASSIGN_FACILITY_TOPIC}" topic`, () => {
    return new Promise((resolve, reject) => {
        mqttClient.subscribe(
            `${ASSIGN_FACILITY_TOPIC}@${mqttClient.options.clientId}`,
            (err) => {
                if (err) {
                    reject(err);
                } else {
...
it(`should subscribe for "${UNASSIGN_FACILITY_TOPIC}" topic`, () => {
    return new Promise((resolve, reject) => {
        mqttClient.subscribe(
            `${UNASSIGN_FACILITY_TOPIC}@${mqttClient.options.clientId}`,
            (err) => {
                if (err) {
                    reject(err);
                } else {
...
it(`should assign the user with login: "${TEST_USER_LOGIN}" to facility with id "${Config.FACILITY_ID}"`, () => {
    const requestId = randomString.generate();
...
it(`should query the facility of the user with login: "${TEST_USER_LOGIN}" where the facility id is: "${Config.FACILITY_ID}"`, () => {
    const requestId = randomString.generate();

    return new Promise((resolve) => {
        ee.once(requestId, (message) => {
            expect(message.status).to.equal(CONST.SUCCESS_STATUS);
            expect(message.facility.facility.id).to.equal(Config.FACILITY_ID);
...
it(`should unassign the user with login: "${TEST_USER_LOGIN}" from facility with id "${Config.FACILITY_ID}"`, () => {
    const requestId = randomString.generate();
    ...
    return new Promise((resolve) => {
        ...
        mqttClient.publish(
            CONST.DH_REQUEST_TOPIC,
            JSON.stringify({
                action: UNASSIGN_FACILITY_ACTION,
                requestId: requestId,
                userId: testUserId,
                facilityId: Config.FACILITY_ID,
            })
...
it(`should check that the user with login: "${TEST_USER_LOGIN}" is unassigned from the facility with id: "${Config.FACILITY_ID}"`, () => {
    const requestId = randomString.generate();
